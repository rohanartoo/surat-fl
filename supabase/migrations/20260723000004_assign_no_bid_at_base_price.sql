-- Allow the last team standing to win at base price when no bid was placed.
--
-- Reachable whenever every other team folds without anyone bidding: in the
-- initial auction (all eligible teams are auto-enrolled, no interest phase)
-- and in mini auctions (2+ interested, the first to act folds immediately).
-- The lot was then stuck — current_bid/current_bidder_id are null so
-- rpc_assign_player raised 'No bid placed yet.', while the remaining team
-- could not fold out either because the turn pointer is null by then. The
-- auction master's only escape was "Return to pool", even though the UI
-- offered an "Assign — <base price>" button.
--
-- This mirrors the existing solo-interest rule (a single interested team wins
-- at base_price), so the sole surviving bidder now wins at base price. The
-- fallback only applies when exactly one active bidder remains; with none, or
-- with more than one, the call is still rejected.
