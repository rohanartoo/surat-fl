-- Defense-in-depth: rpc_assign_player must never hand the player to a team
-- that has folded on the lot.
--
-- Not reachable through normal play — current_bidder_id always tracks the
-- highest remaining bidder, and the no-bid base-price fallback resolves to
-- the sole non-folded survivor — so this guard never rejects a legitimate
-- assignment. It exists so that if any upstream state is ever corrupted
-- (a bad manual DB edit, a future logic change), the assign fails loudly
-- instead of silently awarding a player to a team that withdrew.
--
-- Body is otherwise identical to 20260723000008; only the guard is added,
-- right after the winning team is resolved.
...
begin
  ...
  -- Never assign to a team that has folded on this lot (see migration header).
  select is_folded into v_is_folded from public.bids
    where lot_id = p_lot_id and team_id = v_team_id;
  if v_is_folded is true then
    raise exception 'Cannot assign: the winning team has folded on this lot.';
  end if;
