...
- There is no in-app way to **un-finalize** a gameweek. Once a `gameweek_scoring_status` row exists (written automatically when FPL flips `events[].finished`, or by the admin "Sync & force-finalize" control on /dashboard), the only ways to clear it are `/api/admin/reset`'s full league wipe or manual SQL. Deliberately left out: deleting that row re-locks all seven teams out of their lineups and re-opens the snapshot for the next cron to rebuild from since-changed rosters, which can be worse than the mistake it undoes. Needs its own design if it ever becomes necessary.
