...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    switch (action) {
      ...
      case "place-bid":        return await handlePlaceBid(request)
      case "fold":             return await handleFold(request)
      case "assign-player":    return await handleAssignPlayer(request)
      case "undo-bid":         return await handleUndoBid(request)
      case "undo-last-assignment": return await handleUndoLastAssignment(request)
      case "reset-timer":      return await handleResetTimer(request)
      case "return-to-pool":   return await handleReturnToPool(request)
      case "advance-position": return await handleAdvancePosition(request)
      case "cancel":           return await handleCancel(request)
      case "end-draft":        return await handleEndDraft(request)
      default:
